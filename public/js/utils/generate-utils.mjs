...
function renderOutput(outputImages) {
  if (outputImages.length <= 0) {
    document.querySelector('#output').innerHTML =
      'Click "Generate Image" Button to generate new image. (Clickea el boton "Generar Imagen" para generar nueva imagen)';
  ...
  document.querySelector('#output').innerHTML = outputImages
    ...
    >
      ...
      <div style="text-align: center">
        ...
        ">Download Image (Descargar Imagen)</a>
        <br/>
        <br/>
        ...
        <button 
          ...
          data-index="${index}" 
        >
          Move Left (Mover a Izq.)
        </button>
        <button 
          class="button move-right"
          data-index="${index}" 
        >
          Move Right (Mover a Der.)
        </button>
      </div>
    </div>
    `
